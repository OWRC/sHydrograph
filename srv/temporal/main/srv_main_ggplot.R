  ...
  if (!is.null(v$df)){
    withProgress(message = 'building plot..', value = 0.1, {
      ...
      v$df[v$df$RDNC %in% xs & v$df$IID %in% input$pck.raw,] %>% 
        ...
        geom_path(data = pick(RDNC == "WtrLvl"),aes(colour=IID)) +
        geom_path(data = pick(grp == "Atmospheric Pressure (kPa)"),aes(colour=IID)) +
        geom_area(data = pick(RDNC == "PackDepth"),aes(colour=IID,fill=RDNC)) + #
        geom_path(data = pick(grp == "Stream flow (m³/s)"), aes(colour=IID)) +
        geom_point(data = pick(RDNC == "WtrLvl.s"), aes(colour=IID), shape=23, fill="black", size=2) +
